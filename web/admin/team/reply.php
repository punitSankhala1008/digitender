...
$query="SELECT * FROM ticket  where priority='".$_REQUEST['pri']."' and clientid='".$_REQUEST['clientid']."' and title='".$_REQUEST['title']."'";
...
Department			
			<input type="text" class="form-control" value="<?=$department?>" name="clientid">
                  ...
                  <input type="hidden"  class="form-control" value="<?=$_REQUEST['clientid']?>" name="id">
                </div>
				...
				<div class="form-group">Reply
                 <input type="text" name="reply" value="<?=$reply?>" class="form-control">
				 </div>
...
if(isset($_REQUEST['ticket']))
{
					...
					extract($_REQUEST);
				  $queryy="UPDATE `ticket` SET reply='$reply' where clientid='$id' and title='$title'";
					$n=iud($queryy);
					if($n==1)
					{
